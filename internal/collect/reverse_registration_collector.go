...
type ReverseRegistrationCollectorOption func(*ReverseRegistrationCollector)

func WithReverseRegistrationClient(client *http.Client) ReverseRegistrationCollectorOption {
	return func(c *ReverseRegistrationCollector) {
		if client != nil {
			c.client = client
		}
	}
...
func WithReverseRegistrationJudge(judge ownership.Judge) ReverseRegistrationCollectorOption {
	return func(c *ReverseRegistrationCollector) {
		c.judge = judge
	}
}

func NewReverseRegistrationCollector(options ...ReverseRegistrationCollectorOption) *ReverseRegistrationCollector {
...
func (c *ReverseRegistrationCollector) Process(ctx context.Context, pCtx *models.PipelineContext) (*models.PipelineContext, error) {
	telemetry.Info(ctx, "[Reverse Registration Collector] Processing seeds...")
	...
	for _, seed := range pCtx.CollectionSeeds() {
		...
		lineage.RecordOwnershipJudgeEvaluation(pCtx, "reverse_registration_collector", request, decisions)
