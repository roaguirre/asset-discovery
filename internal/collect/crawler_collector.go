...
type CrawlerCollectorOption func(*CrawlerCollector)

func WithCrawlerClient(client *http.Client) CrawlerCollectorOption {
	return func(c *CrawlerCollector) {
		if client != nil {
			c.client = client
		}
	}
...
func WithCrawlerJudge(judge ownership.Judge) CrawlerCollectorOption {
	return func(c *CrawlerCollector) {
		c.judge = judge
	}
}

func NewCrawlerCollector(options ...CrawlerCollectorOption) *CrawlerCollector {
...
func (c *CrawlerCollector) Process(ctx context.Context, pCtx *models.PipelineContext) (*models.PipelineContext, error) {
	telemetry.Info(ctx, "[Crawler Collector] Processing seeds...")
	...
	for _, seed := range pCtx.CollectionSeeds() {
		...
		for start := 0; start < len(judgeCandidates); start += maxCrawlerJudgeBatchSize {
			...
			lineage.RecordOwnershipJudgeEvaluation(pCtx, "crawler_collector", request, decisions)
			...
			for _, decision := range decisions {
				...
				if !ownership.IsHighConfidence(decision.Confidence) {
					telemetry.Infof(ctx, "[Crawler Collector] Skipping %s due to low-confidence judge decision %.2f.", decision.Root, decision.Confidence)
					continue
				}
