...
type WaybackCollectorOption func(*WaybackCollector)

func WithWaybackClient(client *http.Client) WaybackCollectorOption {
	return func(c *WaybackCollector) {
		if client != nil {
			c.client = client
		}
	}
...
func NewWaybackCollector(options ...WaybackCollectorOption) *WaybackCollector {
...
func (c *WaybackCollector) Process(ctx context.Context, pCtx *models.PipelineContext) (*models.PipelineContext, error) {
	telemetry.Info(ctx, "[Wayback Collector] Processing seeds...")
	...
	for _, seed := range pCtx.CollectionSeeds() {
		...
		for _, baseDomain := range seed.Domains {
			telemetry.Infof(ctx, "[Wayback Collector] Querying CDX Archive for: %s", baseDomain)
