...
func TestJSONExporter_WritesFlatAssetsWithRegistrableDomainMetadata(t *testing.T) {
	...
	if payload[0].Identifier != "example.com" || payload[0].DomainKind != DomainKindRegistrable || payload[0].RegistrableDomain != "example.com" {
		t.Fatalf("expected first JSON row to be registrable domain metadata, got %+v", payload[0])
	}

	if payload[1].Identifier != "api.example.com" || payload[1].DomainKind != DomainKindSubdomain || payload[1].RegistrableDomain != "example.com" {
...
func TestCSVExporter_WritesDomainKindAndRegistrableDomainColumns(t *testing.T) {
	...
	if rows[1][1] != string(DomainKindRegistrable) || rows[1][2] != "example.com" || rows[1][4] != "example.com" {
		t.Fatalf("expected first data row to be grouped registrable domain, got %+v", rows[1])
	}

	if rows[2][1] != string(DomainKindSubdomain) || rows[2][2] != "example.com" || rows[2][4] != "api.example.com" {
