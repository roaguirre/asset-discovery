...
func NewDNSCollector() *DNSCollector {
	...
	return &DNSCollector{
		...
		lookupRDAP: func(ctx context.Context, domain string) (*models.RDAPData, error) {
			return registration.LookupDomain(ctx, rdapClient, domain)
		},
	...
	for _, seed := range pCtx.CollectionSeeds() {
		...
		for _, baseDomain := range seed.Domains {
			...
			for _, issue := range lookupIssues {
				err := issue.asError()
				log.Printf("[DNS Collector] %v", err)
			...
			if len(observation.records) > 0 {
				newAssets = append(newAssets, domainAssetFromObservation(newNodeID("dom"), enum.ID, baseDomain, "dns_collector", observation, nil))
			}
			...
			if len(judgeCandidates) > 0 {
				...
				if err != nil {
					...
					for _, decision := range decisions {
						...
						newAssets = append(newAssets, domainAssetFromObservation(newNodeID("dom-dns-pivot"), enum.ID, decision.Root, source, *pivotCandidate.observation, pivotCandidate.rdap))
						...
						if pCtx.EnqueueSeedCandidate(discoveredSeed, models.SeedEvidence{
							...
							metricsPromoted++
							log.Printf("[DNS Collector] Promoted %s from judged DNS pivots.", decision.Root)
						}
					}
				}
			}
...
func (c *DNSCollector) observeCandidateRoots(ctx context.Context, roots []string, liveCap int, probeKind string) ([]observedDNSCandidate, []error) {
	...
	lookupAggregator.log(probeKind)
...
func (a *dnsLookupIssueAggregator) log(probeKind string) {
